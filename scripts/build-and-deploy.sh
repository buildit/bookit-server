...
if [[ "${TRAVIS_BRANCH}" == "master" ]] && [[ "${TRAVIS_EVENT_TYPE}" == "push" ]]; then
    IMG_VERSION=`node -p -e "require('./package.json').version"`;
    ...
    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_HOME}/ec2/travis app@bookit.riglet.io \
      'docker-compose pull; \
       docker-compose down; \
       AWS_REGION=$(curl -s http://169.254.169.254/latest/dynamic/instance-identity/document | grep '"region"' | cut -d : -f 2) \
